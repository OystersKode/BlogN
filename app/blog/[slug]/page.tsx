...
const BlogDetailPage = async ({ params }: { params: Promise<{ slug: string }> }) => {
  ...
  try {
    ...
    const [session, comments] = await Promise.all([
      getServerSession(authOptions).catch(e => {
          console.error('Session fetching failed on blog page:', e);
          return null;
      }),
      getComments(blog._id.toString())
    ]);
  ...
  return (
    ...
    console.error('Critical failure loading blog page:', error);
    return (
      <main className="min-h-screen flex items-center justify-center bg-white dark:bg-slate-950 px-4">
        <div className="text-center space-y-4 max-w-md">
           <h1 className="text-6xl font-black text-slate-900 dark:text-white">500</h1>
           <p className="text-xl font-bold text-slate-800 dark:text-gray-200">Something went wrong while loading this blog.</p>
           <p className="text-slate-500 dark:text-gray-400 text-sm">This could be a temporary database issue or a configuration error. Please try again later.</p>
           <div className="pt-6">
              <a href="/" className="bg-blue-600 text-white px-8 py-3 rounded-full font-bold shadow-lg hover:shadow-xl transition-all inline-block">Return Home</a>
           </div>
        </div>
      </main>
    );
